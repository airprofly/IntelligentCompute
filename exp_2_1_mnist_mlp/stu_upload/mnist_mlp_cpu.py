...
class MNIST_MLP(object):
    def __init__(self, batch_size=100, input_size=784, hidden1=256, hidden2=128, hidden3=64, out_classes=10, lr=0.01, max_epoch=20, print_iter=100):
        self.batch_size = batch_size
        self.input_size = input_size
        self.hidden1 = hidden1
        self.hidden2 = hidden2
        self.hidden3 = hidden3  # 新增第三层隐藏层
    ...
    def load_data(self):
        ...
        train_labels = self.load_mnist(os.path.join(MNIST_DIR, TRAIN_LABEL), False) ###代码标志位写错
...
def build_mnist_mlp(param_dir='weight.npy'):
    h1,h2,h3,e=256,128,64,20
    mlp=MNIST_MLP(hidden1=h1,hidden2=h2,hidden3=h3,max_epoch=e)
    mlp.load_data()
    mlp.build_model()
    mlp.init_model()
    mlp.train()
    mlp.save_model('mlp-%d-%d-%d-%depoch.npz'%(h1,h2,h3,e))
    mlp.load_model('mlp-%d-%d-%d-%depoch.npz'%(h1,h2,h3,e))
    return mlp
